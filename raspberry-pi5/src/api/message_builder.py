...
async def send_ahrs2():
    await client.mav.ahrs2_send(
        ...
        pitch=0,
        yaw=0,
        altitude=-robot.esp_depth,  #-robot.telemetry_data['depth']
        lat=0,
        lng=0
...
async def send_global_position_int():
    await client.mav.global_position_int_send(
        ...
        lat=int(robot.lat*1e7),
        lon=int(robot.lon*1e7),
        alt=int(abs(robot.esp_depth)), # megdar atman mosbat bashe   int(robot.alt*1e3)
        relative_alt=0,
        vx=0,
        vy=0,
        vz=0,
...
async def send_named_value_float():
    ...
    await client.mav.named_value_float_send(
        time_boot_ms=client.boot_time_ms(),
        name=b"PilotGain",
        value=robot.gear_depth
    )
    await client.mav.named_value_float_send(
        time_boot_ms=client.boot_time_ms(),
        name=b"InputHold",
        value=robot.error_atuo
    )
    await client.mav.named_value_float_send(
        time_boot_ms=client.boot_time_ms(),
        name=b"RollPitch",
    ...
    await client.mav.named_value_float_send(
            time_boot_ms=client.boot_time_ms(),
            name=b"Hamoor", 
            value=44  # مقدار تست – بعداً با داده واقعی جایگزین کن
        )
...
async def send_sys_status():
    await client.mav.sys_status_send(
        ...
        load=0,
        voltage_battery=robot.esp_battery,
        current_battery=-1,
        battery_remaining=-1,
        drop_rate_comm=0,
        errors_comm=0,
